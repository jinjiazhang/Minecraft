...
test('crop HUD distinguishes dry, watered and mature crops',()=>{const r=runtime(),p=r.player('a');r.api.state.plots['20,40']={crop:'parsnip',age:2,water:false};assert.match(r.api.cropHint(p),/还需2个浇水日/);assert.match(r.api.cropHint(p),/今天未浇水/);r.api.state.plots['20,40'].water=true;assert.match(r.api.cropHint(p),/已浇水/);r.api.state.plots['20,40'].age=4;assert.match(r.api.cropHint(p),/已成熟/);});
test('sleep cannot advance day with a stale vote outside the farmhouse',()=>{const r=runtime(),p=r.player('a');r.api.sleepers.add(p.id);p.location={x:140,y:201,z:130};r.api.checkSleep();assert.equal(r.api.state.day,1);});
