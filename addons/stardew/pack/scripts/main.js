import {PICKAXES,BAG_COSTS,ORES,ROOMS,NODES,miner,capacity,load,value,stamp,hitNode,upgrade,sell,unload,roomAt,chamberCommands} from './mining.js';
...
const sleepers=new Set(),forms=new Set(),fishers=new Map(),cooldown=new Map(),hints=new Map(),miningHeld=new Set();
let mineReady=false;
...
function cropHint(p){
 if(roomAt(p.location)>=0){const m=minerData(p);const h=hints.get(p.id);return h&&h.until>system.currentTick?h.text:`${ROOMS[roomAt(p.location)].name} · ${PICKAXES[m.pick].name} · 矿包 ${load(m)}/${capacity(m)} · 日记：回收/升级/返回`; }
...
function menu(p){if(!ready){notify(p,'地图正在搭建，请稍候。');return;}fishers.delete(p.id);miningHeld.delete(p.id);kit(p);const b=[['种子袋 · 选择要播种的作物',()=>later(()=>seedMenu(p))],['背包 / 日志',()=>later(()=>bagMenu(p))],['地图与操作说明',()=>later(()=>guide(p))]];
 ...
 if(near(p,169,12,18)||roomAt(p.location)>=0)b.unshift(['矿工日记 · 下井 / 回收 / 升级',()=>later(()=>mineMenu(p)),'textures/items/iron_pickaxe']);
...
function minerData(p){return miner(String(p.getDynamicProperty('valley:miner')||''));}
function saveMiner(p,m){p.setDynamicProperty('valley:miner',JSON.stringify(m));}
function miningAccess(p){return near(p,169,12,18)||roomAt(p.location)>=0;}
function atMineDesk(p){if(near(p,169,12,18)&&p.location.y>195)return true;notify(p,'请先返回地面矿洞入口办理回收和升级。');return false;}
function mineMenu(p){
 if(!mineReady){notify(p,'矿区正在准备。');return;}if(!miningAccess(p))return;
 miningHeld.delete(p.id);const m=minerData(p),b=[];
 for(const [i,r]of ROOMS.entries())b.push([`${r.name} · ${m.pick>=r.tier?'进入':'需要'+PICKAXES[r.tier].name}`,()=>{
  if(!miningAccess(p))return;if(minerData(p).pick<r.tier){notify(p,'请先升级镐子。');return;}
  p.teleport({x:r.x+1.5,y:171,z:r.z+1.5},{dimension:dim()});const c=p.getComponent('minecraft:inventory')?.container;if(c)for(let k=0;k<9;k++)if(c.getItem(k)?.typeId==='valley:pick')p.selectedSlotIndex=k;
  notify(p,'对准矿石，按住「采矿」连续挖掘；矿包满后用日记回收。');
 },'textures/items/iron_pickaxe']);
 b.push([`立即回收矿包 · ${value(m)}g`,()=>{if(!atMineDesk(p))return;const current=minerData(p),income=sell(current,s);saveMiner(p,current);dirty=true;notify(p,`回收成功 +${income}g，共同资金 ${s.gold}g。`);},'textures/items/gold_ingot']);
 b.push(['矿石存入农场背包 · 留作材料',()=>{if(!atMineDesk(p))return;const current=minerData(p);unload(current,s);saveMiner(p,current);dirty=true;notify(p,'矿石已转入农场背包。');},'textures/items/chest_minecart']);
 if(m.pick<3)b.push([`升级 ${PICKAXES[m.pick+1].name} · ${PICKAXES[m.pick+1].cost}g`,()=>improveMiner(p,'pick'),'textures/items/diamond_pickaxe']);
 if(m.bagLevel<3)b.push([`扩容矿包至 ${capacity(m)+20}格 · ${BAG_COSTS[m.bagLevel+1]}g`,()=>improveMiner(p,'bag'),'textures/items/chest_minecart']);
 b.push(['返回地面矿洞入口',()=>{miningHeld.delete(p.id);p.teleport({x:169.5,y:201,z:19.5},{dimension:dim()});},'textures/items/map_filled']);
 form(p,'矿工日记',`${PICKAXES[m.pick].name} · 矿包 ${load(m)}/${capacity(m)} · 已开采 ${m.broken}块\n${Object.entries(m.bag).map(([id,n])=>ORES[id].name+' ×'+n).join('、')||'矿包为空'}\n泥土硬度1 / 1g；岩石4 / 2g；铜5 / 5g；铁8 / 10g；金12 / 25g；紫水晶16 / 100g；钻石22 / 150g。\n按住开采，矿石破碎后进入个人矿包。镐子与矿包独立升级，消费及回收使用农场共同资金。矿脉约1游戏小时恢复。\n本玩法参考挖矿升级循环，是星露谷的扩展玩法。`,b);
}
function improveMiner(p,kind){if(!atMineDesk(p))return;const m=minerData(p);if(upgrade(m,s,kind)){saveMiner(p,m);dirty=true;notify(p,'升级成功！');}else notify(p,'金币不足或已满级。');}
function prepareMine(){
 s.miningNodes??={};if(!world.getDynamicProperty('valley:mineBuilt')){for(const c of chamberCommands())dim().runCommand(c);world.setDynamicProperty('valley:mineBuilt',true);}
 for(const n of NODES){s.miningNodes[n.key]??={hp:ORES[n.ore].hp,regen:0};dim().getBlock(n)?.setType(s.miningNodes[n.key].hp>0?'minecraft:'+ORES[n.ore].block:'minecraft:air');}
 mineReady=true;dirty=true;console.warn('VALLEY_MINE_READY rooms=4 nodes=24');
}
function mine(p){
 if(!mineReady||roomAt(p.location)<0){hint(p,'§e在矿洞入口打开日记，选择矿层进入。');return;}
 const b=p.getBlockFromViewDirection({maxDistance:6})?.block;const n=b&&NODES.find(n=>n.x===b.location.x&&n.y===b.location.y&&n.z===b.location.z);
 if(!n){hint(p,'§e对准矿石开采，基岩墙和地板不可挖。');return;}
 const m=minerData(p),result=hitNode(m,n,s.miningNodes[n.key],energy(p),stamp(s));hint(p,(result.ok?'§b':'§e')+result.message);
 if(!result.ok){miningHeld.delete(p.id);return;}spend(p,1);saveMiner(p,m);dirty=true;
 if(result.broken){b.setType('minecraft:air');s.xp.mining+=5;try{p.playSound('random.orb',{volume:.3,pitch:1.3});}catch{}}
}
function miningTick(){if(!mineReady)return;for(const p of world.getAllPlayers())if(miningHeld.has(p.id)){
 const selected=p.getComponent('minecraft:inventory')?.container?.getItem(p.selectedSlotIndex)?.typeId;
 if(selected!=='valley:pick'||roomAt(p.location)<0){miningHeld.delete(p.id);continue;}
 if((cooldown.get(p.id)||0)>system.currentTick)continue;cooldown.set(p.id,system.currentTick+6);mine(p);
}}
function refreshMine(){if(!mineReady)return;for(const n of NODES){const d=s.miningNodes[n.key];if(d.hp<=0&&stamp(s)>=d.regen&&!world.getAllPlayers().some(p=>Math.hypot(p.location.x-n.x,p.location.y-n.y,p.location.z-n.z)<2)){d.hp=ORES[n.ore].hp;dim().getBlock(n)?.setType('minecraft:'+ORES[n.ore].block);dirty=true;}}}
...
function dawn(faint){if(!ready)return;if(faint)s.gold-=Math.min(1000,Math.floor(s.gold*.1));const income=nextDay(s);sleepers.clear();fishers.clear();miningHeld.clear();for(const key of Object.keys(s.plots))redraw(key);for(const p of world.getAllPlayers()){p.setDynamicProperty('valley:energy',faint?135:270);p.setDynamicProperty('valley:restDay',`${s.year}:${s.season}:${s.day}`);p.teleport(HOME,{dimension:dim()});notify(p,`${clock()} · 昨日出货 ${income}g${faint?' · 凌晨昏倒，扣除部分医疗费。':''}`);}dim().runCommand(s.rain?'weather rain':'weather clear');save();}
function use(p,id){if(!ready)return;sleepers.delete(p.id);if(id==='journal'){menu(p);return;}if(id==='rod'&&fishers.has(p.id)){fishers.get(p.id).hold=true;return;}if(id==='pick'&&roomAt(p.location)>=0){miningHeld.add(p.id);return;}if((cooldown.get(p.id)||0)>system.currentTick)return;cooldown.set(p.id,system.currentTick+5);
 ...
 if(id==='pick'&&near(p,170,10,24)){mineMenu(p);return;}
...
world.afterEvents.itemStopUse.subscribe(e=>{miningHeld.delete(e.source.id);const f=fishers.get(e.source.id);if(f)f.hold=false;});
world.afterEvents.playerSpawn.subscribe(e=>system.runTimeout(()=>enter(e.player),40));
world.afterEvents.playerLeave.subscribe(e=>{sleepers.delete(e.playerId);fishers.delete(e.playerId);forms.delete(e.playerId);cooldown.delete(e.playerId);hints.delete(e.playerId);miningHeld.delete(e.playerId);});
...
world.afterEvents.worldLoad.subscribe(()=>system.runTimeout(()=>{
 ...
 for(const key of Object.keys(s.plots))redraw(key);renderBuildings();spawnAnimals();prepareMine();ready=true;save();dim().runCommand('setworldspawn 32 201 27');dim().runCommand(s.rain?'weather rain':'weather clear');for(const p of world.getAllPlayers())enter(p);console.warn('VALLEY_READY buildings=10 crops=7');
...
system.runInterval(miningTick,2);system.runInterval(second,20);system.runInterval(fishingTick,2);
system.afterEvents.scriptEventReceive.subscribe(e=>{if(e.id!=='valley:smoke'||e.sourceEntity)return;try{if(!ready)throw Error('not ready');for(const id of ['journal','hoe','seeds','water','harvest','axe','pick','rod'])new ItemStack('valley:'+id);for(const c of Object.values(CROPS))BlockPermutation.resolve('minecraft:'+c.block,{growth:7});BlockPermutation.resolve('minecraft:farmland',{moisturized_amount:7});if(dim().getBlock({x:32,y:200,z:27})?.typeId!=='minecraft:oak_planks')throw Error('farmhouse missing');if(dim().getEntities({tags:['valley_marker']}).length!==17)throw Error('markers missing');if(!mineReady||NODES.some(n=>!dim().getBlock(n)))throw Error('mine nodes unavailable');if(dim().getBlock({x:140,y:169,z:36})?.typeId!=='minecraft:bedrock')throw Error('mine floor missing');console.warn('VALLEY_SMOKE_PASS tools=8 crops=7 markers=17 mineNodes=24');}catch(err){console.warn('VALLEY_SMOKE_FAIL '+err);}});
