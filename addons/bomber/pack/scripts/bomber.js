...
export function isBombItem(item) { return item?.typeId === "minecraft:tnt" && item.nameTag === BOMB_NAME; }
...
const votes = new Set();
let onRoundEnd = () => { };
export function setRoundEndHandler(handler) { onRoundEnd = handler; }
const painted = new Map();
const dim = () => world.getDimension("overworld");
const point = (x, z) => ({ x: BASE.x + x * 2 + 1, y: BASE.y + 1, z: BASE.z + z * 2 + 1 });
const cell = (p) => ({ x: Math.floor((p.x - BASE.x) / 2), z: Math.floor((p.z - BASE.z) / 2) });
const say = (s) => { for (const { player } of seats.values())
    if (player.isValid)
        player.sendMessage(`§6[炸弹人]§r ${s}`); };
export function inBomber(p) { return seats.has(p.id); }
...
function render() {
    ...
    for (let x = 0; x < WIDTH; x++)
        for (let z = 0; z < HEIGHT; z++) {
            ...
            if (bomb)
                fill(x * 2, 1, z * 2, x * 2, 1, z * 2, "red_concrete");
            painted.set(k, signature);
        }
}
function begin() {
    if (!built || preparing || seats.size !== 2)
        return;
    match = new BomberMatch([...seats.keys()]);
    votes.clear();
    render();
    let i = 0;
    for (const s of seats.values()) {
        lobby(s, i++);
        s.sneaking = s.player.isSneaking;
        giveBombs(s.player);
    }
    startAt = system.currentTick + 60;
    say("§e准备！3 秒后开局。红色地板是炸弹，可以离开，但不能再走回去。橙色地板是危险火焰！");
}
export function joinBomber(p) {
    if (seats.has(p.id)) {
    ...
    if (seats.size >= 2) {
        if (!waiting.has(p.id))
            p.sendMessage("§e竞技场已满，已进入等候区；有空位后自动加入。");
        waiting.set(p.id, p);
    ...
    if (built) {
        lobby(seats.get(p.id), seats.size - 1);
        begin();
        return;
    }
    initializeArena();
...
export function initializeArena() {
    ...
    try {
        ...
        catch { /* previous game not installed */ }
        try {
            dim().runCommand("tickingarea remove bomber_arena");
        }
        catch { /* first load */ }
    ...
    system.runTimeout(() => {
        try {
            ...
            match = new BomberMatch(["preview-a", "preview-b"]);
            painted.clear();
            render();
            match = undefined;
            built = true;
...
function fail(e) {
    ...
    for (const [id, s] of seats)
        if (s.player.isValid)
            waiting.set(id, s.player);
    seats.clear();
    votes.clear();
}
export function leaveBomber(id) {
    waiting.delete(id);
    ...
    if (match && !match.ended)
        say("伙伴离开，本局取消。等另一位玩家加入后重新开局。");
    match = undefined;
    votes.clear();
    if (built)
        for (const remaining of seats.values())
            if (remaining.player.isValid)
                lobby(remaining, 0);
}
export function rematch(p) {
    if (!seats.has(p.id) || !match?.ended)
        return;
    votes.add(p.id);
    say(`${p.name} 已准备再来一局（${votes.size}/2）。`);
    if (votes.size === 2)
        begin();
}
export function bomberTick() {
    try {
        if (!built && !preparing && system.currentTick % 100 === 0)
            initializeArena();
        if (built)
            for (const [id, p] of waiting) {
                ...
                if (seats.size < 2)
                    ...
                    if (system.currentTick % 20 === 0)
                        p.onScreenDisplay.setActionBar("§e竞技场已满 · 等待空位后自动加入");
                }
            }
        for (const [id, s] of seats)
            if (!s.player.isValid)
        ...
        for (const s of seats.values()) {
            ...
            if (preparing) {
                p.onScreenDisplay.setActionBar("§e正在准备炸弹人竞技场…");
                continue;
            }
            const fighter = match?.players.find(f => f.id === p.id);
            const locked = !match || match.ended || system.currentTick < startAt;
            if (locked) {
                p.teleport(s.last, { dimension: dim(), keepVelocity: false });
            }
            else if (fighter) {
                const loc = p.location;
                if (p.dimension.id !== dim().id || loc.y < BASE.y + 1 || loc.y > BASE.y + 1.5 || !match.move(p.id, cell(loc)))
                    p.teleport(s.last, { dimension: dim(), keepVelocity: false });
                else
                    s.last = loc;
            }
            ...
            if (system.currentTick % 10 === 0)
                p.onScreenDisplay.setActionBar(!match ? "§e等待伙伴上线 · 两人到齐自动开局" : match.ended ? "§6本局结束 · /menu → 再来一局" : system.currentTick < startAt ? `§e${Math.ceil((startAt - system.currentTick) / 20)} 秒后开始` : `§b炸弹 ${[...match.bombs.values()].filter(b => b.owner === p.id).length}/${fighter.capacity} §e火力 ${fighter.range} §f剩余 ${Math.ceil((ROUND - match.time) / 20)} 秒 §7| 选中 TNT 放置 · 5秒引信`);
        }
        if (!match || match.ended || system.currentTick < startAt)
            return;
        const before = [...match.bombs.values()];
        match.step();
        render();
        explosionEffects(dim(), BASE, match, before);
        if (match.bombs.size < before.length)
            for (const s of seats.values())
                s.player.playSound("random.explode", { volume: .5 });
        if (match.ended) {
            ...
            for (const s of seats.values())
                s.player.onScreenDisplay.setTitle(`§6${result}`, { subtitle: "即将打开重新开始菜单", stayDuration: 100, fadeInDuration: 5, fadeOutDuration: 20 });
        }
    }
    catch (e) {
        fail(e);
...
export function giveBombs(p) {
    ...
    for (let i = 0; i < container.size && stacks < 4; i++) {
        ...
        if (old && !isBombItem(old))
            continue;
        const item = new ItemStack("minecraft:tnt", 64);
        item.nameTag = BOMB_NAME;
    ...
    if (selected >= 0)
        p.selectedSlotIndex = selected;
    p.sendMessage(stacks ? `§e本局已补给 ${stacks * 64} 颗炸弹，选中 TNT 点击地面放置。` : "§e背包已满，请空出格子，然后从 /menu 补领炸弹。");
}
export function placeBombItem(p, location, slot) {
    if (!p.isValid || !match || match.ended || system.currentTick < startAt || p.dimension.id !== dim().id || location.y !== BASE.y + 1)
        return false;
    const container = p.getComponent("minecraft:inventory")?.container;
    const item = container?.getItem(slot);
    if (!container || !isBombItem(item) || !item)
        return false;
    if (!match.place(p.id, cell(location))) {
        p.onScreenDisplay.setActionBar("§e这里不能放置，或同时放置的炸弹已达上限");
        return false;
    }
    if (item.amount > 1) {
        item.amount--;
