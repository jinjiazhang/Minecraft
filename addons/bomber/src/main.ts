...
system.beforeEvents.startup.subscribe(event => {
  event.customCommandRegistry.registerCommand({ name: "bomber:menu", description: "炸弹人规则与再来一局", permissionLevel: CommandPermissionLevel.Any, cheatsRequired: false }, origin => {
    ...
    system.run(() => { if (p.isValid) requestMenu(p); });
    return { status: CustomCommandStatus.Success };
  });
});
world.afterEvents.worldLoad.subscribe(() => {
  initializeArena();
  for (const p of world.getAllPlayers()) joinBomber(p);
});
...
world.beforeEvents.playerInteractWithBlock.subscribe(e => {
  e.cancel = true;
  if (!e.isFirstEvent || !isBombItem(e.itemStack)) return;
  const offsets = {
...
system.afterEvents.scriptEventReceive.subscribe(e => {
  if (e.id === "bomber:menu" && e.sourceEntity instanceof Player) requestMenu(e.sourceEntity);
});
