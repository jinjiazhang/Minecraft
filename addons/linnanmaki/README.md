...
### 2026-09-12：末日赫尔辛基玩法已接入

当前官方地图同时加载 `addons/deadcity`，玩家进入乐园幸存者基地后用 `/dead:menu` 开始搜集与撤离。已切换冒险模式、普通难度，资源包必须下载。玩法详情与部署见 [deadcity README](../deadcity/README.md)。下方官方地图单包部署记录为此前状态，不能用旧安装脚本覆盖当前激活清单。
