...
function horizontalDistance(player, target) {
...
function buildCourse(dimension) {
...
function getState(player) {
    ...
    const created = { stage: 0, busy: false };
...
function askRiddle(player, state) {
    state.busy = true;
...
function askPassword(player, state) {
    state.busy = true;
    const form = new ModalFormData()
        .title("铁栏小室")
...
export function isHunting(player) {
    const state = progress.get(player.id);
