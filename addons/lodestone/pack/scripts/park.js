...
export function isMenuStand(course, x, y, z) {
...
export function openHole(player, x, y, z) {
    fill(player, x - 1, y, z, x + 1, y + 2, z, "minecraft:air");
}
function road(player, x1, z1, x2, z2, y, block) {
    fill(player, x1, y - 1, z1, x2, y - 1, z2, block);
    fill(player, x1, y, z1, x2, y + 3, z2, "minecraft:air");
}
function house(player, cx, y, cz, rx, rz, wall, floor, doorZ) {
...
function flattenJobs(player, hub) {
    const jobs = [];
...
function runJobs(jobs, index, done) {
...
function buildCandyPort(player, hub) {
    const { x: cx, y, z: cz } = hub;
...
export function prepareCandyPort(player, existing, done) {
