...
system.beforeEvents.startup.subscribe(event => {
    event.customCommandRegistry.registerCommand({ name: "lodestone:menu", description: "打开地心探险手册：提示、集合与参观", permissionLevel: CommandPermissionLevel.Any, cheatsRequired: false }, origin => {
        ...
        if (!(p instanceof Player))
            return { status: CustomCommandStatus.Failure, message: "请在游戏中打开手册。" };
        system.run(() => requestMenu(p));
        return { status: CustomCommandStatus.Success };
    });
});
system.afterEvents.scriptEventReceive.subscribe(event => {
    if (event.id === "lodestone:menu" && event.sourceEntity instanceof Player)
        requestMenu(event.sourceEntity);
});
world.beforeEvents.playerInteractWithBlock.subscribe(event => {
    if (event.player.dimension.id !== "minecraft:overworld")
        return;
...
world.afterEvents.playerSpawn.subscribe(event => {
    ...
    system.run(() => { if (p.isValid)
        protect(p); });
    system.runTimeout(() => { if (p.isValid)
        join(p); }, 10);
});
