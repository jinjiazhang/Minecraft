...
function station(room: number, id: string, x: number, z: number, label: string, block: string, text?: string): void { stations.push({ room, id, x, z, label, block, text }); }
for (let r = 0; r < 6; r++) {
...
for (let r = 0; r < 6; r++) station(r, "story", -9, 10, "探险故事牌\n触摸读一读", "chiseled_bookshelf", stories[r]);
export function stationAt(p: Vector3): Station | undefined { return stations.find(s => { const v = at(s.room, s.x, 0, s.z); return p.x === v.x && p.z === v.z && p.y >= 80 && p.y <= 83; }); }

export type Job = { kind: "fill"; a: Vector3; b: Vector3; block: string } | { kind: "sign"; p: Vector3; text: string };
export function execute(d: Dimension, job: Job): void {
  if (job.kind === "fill") { const { a, b } = job; d.runCommand(`fill ${a.x} ${a.y} ${a.z} ${b.x} ${b.y} ${b.z} minecraft:${job.block}`); return; }
  const b = d.getBlock(job.p);
  if (!b) throw new Error(`Sign chunk unavailable: ${JSON.stringify(job.p)}`);
  ...
  if (!sign) throw new Error("Sign component unavailable");
  sign.setText(job.text); sign.setWaxed(true);
}

// Every fill is <= 32,768 blocks. Generated deterministically for restart/retry safety.
export function buildPlan(): Job[] {
  const jobs: Job[] = [];
  let r = 0;
  const box = (x: number, y: number, z: number, xx: number, yy: number, zz: number, block: string) => jobs.push({ kind: "fill", a: at(r, x, y, z), b: at(r, xx, yy, zz), block });
  const put = (x: number, y: number, z: number, block: string) => box(x, y, z, x, y, z, block);
  const sign = (x: number, y: number, z: number, text: string) => jobs.push({ kind: "sign", p: at(r, x, y, z), text });
  const lamp = (x: number, z: number) => { put(x, 0, z, "stone_bricks"); box(x, 1, z, x, 3, z, "oak_fence"); put(x, 4, z, "sea_lantern"); };
  const mushroom = (x: number, z: number, h: number, color: string) => {
    box(x, 0, z, x, h, z, "mushroom_stem"); box(x - 2, h, z - 2, x + 2, h, z + 2, color);
    box(x - 1, h + 1, z - 1, x + 1, h + 1, z + 1, color); put(x, h - 1, z + 1, "shroomlight");
    put(x - 1, h + 1, z, "white_wool"); put(x + 1, h, z + 2, "white_wool");
  };
  const crystal = (x: number, z: number, h: number, color: string) => {
    box(x - 1, 0, z - 1, x + 1, 1, z + 1, "calcite");
    box(x, 1, z, x, h, z, color); box(x + 1, 1, z, x + 1, Math.max(2, h - 2), z, "amethyst_block"); put(x, 0, z, "sea_lantern");
  };
  ...
  const house = (x: number, z: number) => {
    box(x - 4, -1, z - 3, x + 4, -1, z + 3, "spruce_planks"); box(x - 4, 0, z - 3, x + 4, 4, z + 3, "stripped_oak_log");
    box(x - 3, 0, z - 2, x + 3, 3, z + 2, "air"); box(x - 1, 0, z + 3, x + 1, 2, z + 3, "air");
    for (const dx of [-3, 3]) box(x + dx, 1, z + 3, x + dx, 2, z + 3, "glass");
    for (let h = 0; h < 4; h++) box(x - 5 + h, 5 + h, z - 4, x + 5 - h, 5 + h, z + 4, "dark_oak_planks");
    put(x - 2, 0, z - 1, "crafting_table"); put(x + 2, 0, z - 1, "bookshelf"); put(x, 3, z, "lantern");
  ...
  for (r = 0; r < 6; r++) {
    ...
    box(-15, 16, -15, 15, 21, 15, "air"); box(-11, 22, -11, 11, 25, 11, "air");
    ...
    for (let t = -16; t <= 16; t += 8) {
      ...
      put(t, 6, -18, "glowstone"); put(-19, 5, t, "glowstone"); put(19, 5, t, "glowstone");
      box(t, 0, 19, t + 1, 7, 20, "tuff");
    }
    for (let k = 0; k < 18; k++) {
      const x = (k * 7) % 21 - 10, z = (k * 11) % 21 - 10;
      put(x, 22 + (k % 3), z, "sea_lantern");
      if (k % 3 === 0) box(x, 19, z, x, 21, z, "oak_leaves");
    }
    box(-1, -1, -15, 1, -1, 17, r === 1 ? "polished_andesite" : "smooth_sandstone");
    box(-15, -1, 7, 15, -1, 9, "smooth_sandstone");
    for (let z = -13; z <= 13; z += 6) { put(-2, -1, z, "sea_lantern"); put(2, -1, z, "sea_lantern"); }
    lamp(-16, 13); lamp(16, 13);
    sign(0, 0, 12, `${TITLES[r]}\n沿灯光小路前进\n绿台提示 · 金台手册\n触摸彩色台子互动`);
    if (r === 0) {
      tree(0, -7, false); house(-12, -10); house(12, -10);
      box(-5, -1, -2, 5, -1, 5, "stone_bricks");
      for (const x of [-15, -10, 10, 15]) { put(x, 0, 3, "flowering_azalea"); put(x, 0, 5, "red_tulip"); }
      sign(0, 0, 1, "星灯大树睡着了\n去地心寻找光之种\n和伙伴一起出发！");
    } else if (r === 1) {
      for (const x of [-15, 15]) { box(x, 0, -14, x, 4, -6, "bookshelf"); box(x - 1, 0, -4, x + 1, 0, -2, "crafting_table"); }
      // Brass drill vehicle: caterpillar tracks, observation cabin, layered drill nose.
      box(-4, 0, -8, 4, 2, 0, "copper_block");
      for (const x of [-5, 5]) { box(x, 0, -8, x, 1, 0, "deepslate_tiles"); for (let z = -7; z <= -1; z += 2) put(x, 1, z, "polished_blackstone"); }
      box(-3, 3, -6, 3, 5, -1, "cut_copper"); box(-2, 3, -5, 2, 4, -1, "glass"); box(-2, 5, -5, 2, 5, -2, "cut_copper");
      box(-2, 1, -10, 2, 3, -9, "iron_block"); box(-1, 1, -12, 1, 3, -11, "iron_block"); put(0, 2, -13, "lightning_rod");
      put(-3, 2, 1, "sea_lantern"); put(3, 2, 1, "sea_lantern");
      for (let z = -16; z <= 9; z++) { put(-5, -1, z, "iron_block"); put(5, -1, z, "iron_block"); }
      box(11, 1, -15, 11, 10, -15, "copper_block"); box(-12, 9, -15, 11, 9, -15, "cut_copper");
    } else if (r === 2) {
      for (const [x, z, h, c] of [[-14,-12,8,"red_mushroom_block"],[9,-14,10,"blue_wool"],[14,3,7,"yellow_wool"],[-15,7,5,"red_mushroom_block"],[-7,-15,6,"red_mushroom_block"]] as [number,number,number,string][]) mushroom(x,z,h,c);
      box(-15, -1, -2, -9, -1, 3, "spruce_planks");
      for (let x = -14; x <= 14; x += 4) { put(x, 0, -17, "flowering_azalea"); put(x, 0, 17, "azalea"); }
      // Shape mosaics supplement colors for early readers.
      put(-6, 3, -11, "white_wool"); box(0, 3, -11, 4, 3, -11, "blue_wool"); put(2, 2, -11, "mushroom_stem");
      box(9, 3, -11, 11, 3, -11, "yellow_wool"); box(10, 2, -11, 10, 4, -11, "yellow_wool");
    } else if (r === 3) {
      // Covered channels: children never need to swim or risk drowning.
      for (const x of [-7, 7]) { box(x - 1, -2, -15, x + 1, -2, 12, "prismarine"); box(x - 1, -1, -15, x + 1, -1, 12, "light_blue_stained_glass"); }
      box(-14, -1, 5, 14, -1, 8, "dark_oak_planks");
      for (const x of [-16, 16]) { box(x, 0, -15, x + 1, 9, -15, "prismarine_bricks"); box(x, 0, -7, x + 1, 9, -7, "prismarine_bricks"); box(x, 9, -15, x + 1, 9, -7, "dark_prismarine"); }
      // Eight-spoke monumental wheel in a vertical plane.
      for (let x = -6; x <= 6; x++) for (let y = 1; y <= 13; y++) {
        const radius = Math.hypot(x, y - 7);
        if ((radius >= 5 && radius <= 6.2) || x === 0 || y === 7 || Math.abs(x) === Math.abs(y - 7)) put(x, y, -12, "dark_oak_log");
      }
      put(0, 7, -11, "copper_block"); box(-3, 0, -16, 3, 2, -14, "prismarine");
      sign(0, 0, 4, "水渠藏在玻璃下面\n放心走在桥上\n左右水闸各开一次");
    } else if (r === 4) {
      for (const [x,z,h] of [[-14,-13,10],[-7,-15,7],[4,-14,12],[14,-12,8],[16,4,6],[-15,7,5]]) crystal(x,z,h,"purple_stained_glass");
  ...
  for (const s of stations) {
    r = s.room; put(s.x, 0, s.z, "chiseled_stone_bricks"); put(s.x, 1, s.z, s.block);
    sign(s.x, 2, s.z, s.label); put(s.x, -1, s.z + 1, "sea_lantern");
  }
  return jobs;
}

export function visualPlan(q: Quest): Job[] {
  const jobs: Job[] = [];
  const put = (room: number, x: number, y: number, z: number, block: string) => jobs.push({ kind: "fill", a: at(room,x,y,z), b: at(room,x,y,z), block });
  for (const s of stations) {
    if (["help", "travel", "story", "read"].includes(s.id)) continue;
    let lit = q.stage > s.room || !!q.pairs[s.id] || q.parts.includes(s.id);
    if (s.room === q.stage && (q.stage === 2 || q.stage === 4)) lit = (q.stage === 2 ? ["red","blue","yellow"] : ["violet","cyan","pink"]).slice(0,q.step).includes(s.id);
    put(s.room, s.x, 0, s.z, lit ? "sea_lantern" : "chiseled_stone_bricks");
  }
  for (let side = 0; side < 2; side++) if (q.stage > 3 || q.pairs[`valve${side}`]) {
    const x = side ? 7 : -7;
    jobs.push({ kind: "fill", a: at(3,x-1,-2,-15), b: at(3,x+1,-2,12), block: "water" });
    for (let z = -14; z < 12; z += 4) put(3, x + (side ? 2 : -2), 0, z, "sea_lantern");
  }
  if (q.stage >= 4) put(3, 0, 7, -11, "sea_lantern");
  if (q.stage >= 6) {
