...
function near(player: Player, target: Site, range: number): boolean {
...
function buildGate(player: Player, x: number, y: number, z: number, block: string): void {
  fill(player, x - 2, y, z, x + 2, y + 2, z, block);
  put(player, x, y, z, "minecraft:air");
}

function openGate(player: Player, x: number, y: number, z: number): void {
  fill(player, x - 1, y, z, x + 1, y + 2, z, "minecraft:air");
}

function playerHasItem(player: Player, typeId: string): boolean {
...
function takeItem(player: Player, typeId: string): boolean {
...
function isAnswer(value: unknown, accepted: string[]): boolean {
  const answer = normalizeAnswer(value).toLowerCase();
  return accepted.includes(answer);
}

function buildPark(player: Player): Course {
...
function getState(player: Player): HuntState {
  ...
  const created: HuntState = { stage: 0, busy: false, cooldown: 0 };
...
function ensureParkOnJoin(player: Player): void {
  if (course) {
    player.teleport(asVector(course.start));
