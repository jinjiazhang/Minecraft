...
export interface Quest { version: 2; stage: number; parts: string[]; pairs: Record<string, string>; step: number; reader: string; roster: string[]; }
export function freshQuest(): Quest { return { version: 2, stage: 0, parts: [], pairs: {}, step: 0, reader: "", roster: [] }; }
export function validQuest(v: unknown): v is Quest {
  const q = v as Quest;
...
export interface Outcome { message: string; changed?: boolean; advanced?: boolean; }
export function act(q: Quest, who: string, id: string): Outcome {
  if (!q.roster.includes(who)) return { message: "这是两位探险员的机关。你可以陪同参观和帮忙读提示。" };
  const advance = (): Outcome => { q.stage++; q.step = 0; q.reader = ""; q.pairs = {}; return { message: `完成！下一站：${TITLES[q.stage]}`, changed: true, advanced: true }; };
  const pair = (prefix: string): Outcome => {
    if (id !== `${prefix}0` && id !== `${prefix}1`) return { message: "先看看这一关的任务提示。" };
    if (q.pairs[id]) return { message: "这一边已经完成啦，请让伙伴操作另一边。" };
    if (Object.values(q.pairs).includes(who)) return { message: "你的一半已经完成，另一半留给伙伴！" };
    q.pairs[id] = who;
    return Object.keys(q.pairs).length === 2 ? advance() : { message: "你的一半完成了！等伙伴完成另一边，不用着急。", changed: true };
  };
  if (q.stage === 0) return pair("ready");
  if (q.stage === 1) {
    if (["gear", "lens", "battery"].includes(id)) {
      if (q.parts.includes(id)) return { message: "这个零件已经装好了，找找另外两个。" };
      q.parts.push(id); return { message: `装好零件！${q.parts.length}/3。${q.parts.length === 3 ? "请两个人分别操作启动台。" : "沿小路找下一座零件台。"}`, changed: true };
    }
    if (q.parts.length < 3) return { message: "钻地车还缺零件，先找齐三个亮灯的零件台。" };
    return pair("engine");
  }
  if (q.stage === 2 || q.stage === 4) {
    ...
    if (id === "read") { q.reader = who; return { message: `§e第 ${q.step + 1} 步：${words[q.step]}。§r请告诉伙伴，让伙伴去触摸它。`, changed: true }; }
    if (!order.includes(id)) return { message: "触摸阅读台或颜色台来解谜。" };
    if (!q.reader) return { message: "先请伙伴触摸阅读台，看看这一小步的线索。" };
    if (q.reader === who) return { message: "你看到了线索，把它告诉伙伴，让伙伴操作吧。" };
    if (id !== order[q.step]) return { message: "差一点！已完成的部分还在，再问问伙伴颜色和图案。" };
    q.step++; q.reader = "";
    return q.step === 3 ? advance() : { message: `点亮了 ${q.step}/3！请再读一次线索，也可以交换分工。`, changed: true };
  }
  if (q.stage === 3) return pair("valve");
  if (q.stage === 5) return pair("plant");
