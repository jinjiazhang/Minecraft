...
import { near, openHole, Site } from "./park";
import { course, grantToken, hasToken, HuntState, plateIndex } from "./state";
...
function tickKeys(player: Player, state: HuntState): void {
...
export function tickPuzzles(player: Player, state: HuntState): void {
  if (!course) {
    return;
  }
