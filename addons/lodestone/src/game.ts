...
import { act, freshQuest, HINTS, OBJECTIVES, Quest, TITLES, validQuest } from "./quest";
import { at, BASE, buildPlan, execute, Job, MAP_VERSION, roomOf, spawn, Station, stations, visualPlan } from "./scenes";
...
function say(message: string): void { for (const p of world.getAllPlayers()) p.sendMessage(`§b[地心探险队]§r ${message}`); }
function load(): void {
  if (booted) return;
  const raw = world.getDynamicProperty(SAVE);
  if (typeof raw === "string") {
    const data: unknown = JSON.parse(raw);
    if (!validQuest(data)) throw new Error("冒险进度格式异常，已保留原始存档，请检查服务端日志。");
    quest = data;
  }
  booted = true;
}
...
function runJobs(jobs: Job[], done: () => void, failed: (e: unknown) => void): void {
  ...
  const step = () => {
    try {
      for (let n = 0; n < 4 && index < jobs.length; n++, index++) { execute(dim(), jobs[index]); retries = 0; }
      if (building && system.currentTick % 40 < 4) for (const p of world.getAllPlayers()) p.onScreenDisplay.setActionBar(`正在布置地心世界 · ${Math.floor(index / jobs.length * 100)}% · 请稍候`);
      if (index === jobs.length) { done(); return; }
      system.runTimeout(step, 1);
    } catch (e) {
      if (++retries <= 8) system.runTimeout(step, 20);
      else { console.error(`EARTH_BUILD job ${index}: ${JSON.stringify(jobs[index])}: ${e}`); failed(e); }
    }
  };
  step();
}
export function checkpoint(): number { return quest.stage >= 6 ? 0 : quest.stage; }
...
export function refreshVisuals(): void {
  ...
  if (lastVisual === signature) return;
  // Mark only after successful writes; retry on a later tick if any block is unavailable.
  for (const job of visualPlan(quest)) execute(dim(), job);
  lastVisual = signature;
}
export function join(p: Player): void {
  try {
    load();
...
export function initialize(): void {
  ...
  try {
    ...
    try { dim().runCommand("tickingarea remove earth_adventure"); } catch { /* first installation */ }
  ...
  const fail = (e: unknown) => { building = false; ready = false; say("场景准备中断，原进度已保存。输入 /lodestone:menu，点「重试准备」。"); console.error(`EARTH_INIT ${e}`); };
  const finish = () => {
    try {
      refreshVisuals();
      world.setDynamicProperty(MAP, MAP_VERSION); world.setDefaultSpawnLocation(spawn(0));
      building = false; ready = true; lastProgress = system.currentTick;
      for (const p of world.getAllPlayers()) returnToCamp(p);
  ...
  system.runTimeout(() => {
    try {
      if (world.getDynamicProperty(MAP) === MAP_VERSION) { runJobs(visualPlan(quest), finish, fail); return; }
      say("正在建造六座地心场景，首次准备约需一到两分钟。可以先在原地等候。");
      runJobs(buildPlan(), finish, fail);
    } catch (e) { fail(e); }
  }, 80);
}
export function hint(p: Player, direct = false): void {
...
export function interact(p: Player, s: Station): void {
  if (!ready || building || !p.isValid || p.dimension.id !== "minecraft:overworld") return;
  if ((cooldown.get(p.id) ?? 0) > system.currentTick) return;
  cooldown.set(p.id, system.currentTick + 8);
  if (s.id === "help") { hint(p); return; }
  if (s.id === "story") { p.sendMessage(s.text!); return; }
  if (s.room !== quest.stage) { p.sendMessage("§e这一站可以自由参观。打开金色旅程台返回当前任务。"); return; }
  const before = quest.stage;
  const result = act(quest, p.name, s.id);
  p.sendMessage(result.message);
  if (result.changed) {
    save(); lastProgress = system.currentTick;
    try { refreshVisuals(); } catch (e) { console.warn(`EARTH_VISUAL ${e}`); }
    p.playSound("random.orb");
  }
  if (result.advanced) {
    say(`§6${TITLES[before]}完成！§r${OBJECTIVES[quest.stage]}`);
    for (const friend of participants()) { friend.playSound("random.levelup"); friend.onScreenDisplay.setTitle("§6合作成功！", { subtitle: `下一站：${TITLES[quest.stage]}`, stayDuration: 60, fadeInDuration: 10, fadeOutDuration: 20 }); }
    const expected = quest.stage;
    system.runTimeout(() => {
      if (quest.stage !== expected) return;
      for (const friend of world.getAllPlayers()) returnToCamp(friend);
      if (quest.stage === 6) say("§e两颗种子长成了发光树！左右两间小屋是你们的探险基地。金色台子可以重游六个场景。");
    }, expected === 6 ? 180 : 80);
  }
}
...
export function tick(): void {
  ...
  for (const p of world.getAllPlayers()) {
    try {
      ...
      if (room < 0 || room > quest.stage) { returnToCamp(p); continue; }
      ...
      if (visits.get(p.id) !== room) { visits.set(p.id, room); p.sendMessage(`§b${TITLES[room]}§r：触摸绿色提示台获取帮助。`); }
      if (system.currentTick - lastProgress > 1200 && system.currentTick % 600 === 0) p.sendMessage(`§a小提示：§r${HINTS[quest.stage][0]} §7还需要帮助就触摸绿台。`);
    } catch (e) { console.warn(`EARTH_PLAYER ${p.name}: ${e}`); }
  }
}
